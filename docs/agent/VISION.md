# Code Monkeys Vision: The Solo Product Factory
...
## The Code Monkeys  Trinity

| Layer | Product | Job |
|-------|---------|-----|
| **Bootstrap** | **Code Monkeys Init** | 0 → working repo with CI/release wiring |
| **Front Half** | **Code Monkeys Dev** | Constitution → Spec → Plan → Tasks → Analyze → Implement → Secure → Tests |
| **Back Half** | **Code Monkeys Ship** | Heal → Docs → Ship → Publish → Announce → Monitor|

### Code Monkeys Init — "Factory Bootstrap"
...
- `.Code Monkeys ship/config.toml` tuned to your environment

### Code Monkeys Dev — "Front-Half Loop" (spec-kit fork/wrapper)
- Spec authoring
- Task decomposition
- TDD scaffolding
- Traceability mapping (req → code/tests/docs)

### Code Monkeys Ship — "Back-Half Loop"
...
Code Monkeys Ship is **not a chatbot**. It's an **execution engine with governance**:
